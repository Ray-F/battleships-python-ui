...
# Set to true if operating system is windows
...
class SplashScreen(tk.Frame):
    def __init__(self, parent):
        ...
        # Title (aesthetic)
        self.title = tk.Label(self, text="BATTLESHIPS", fg=theme.GRAY_BLACK, bg=theme.WHITE,
                              font=("Tw Cen MT", 75 if windows else 100, "bold"))
        ...
        info_canvas = tk.Canvas(self.main_frame, width=480, height=320, highlightthickness=0, bg=theme.WHITE)
        ...
        reset_score_button = CustomButton(info_canvas, text="Reset Scores", width=90, height=40, font=("Tw Cen MT", 9 if windows else 12),
        ...
        buttons_container = tk.Frame(self.main_frame, width=256, height=int(self.main_frame["height"]), bg=theme.WHITE)
    ...
    def __init__(self, parent):
        ...
        for i in range(3):
            ...
            id_label = tk.Label(save_frame, text="{}SAVE #{}".format("" if save else "EMPTY ", i + 1),
                                font=("Tw Cen MT", 9 if windows else 12))
            ...
            if save:
                ...
                # The date
                symbol_space = "#" if windows else "-"
                date_label = tk.Label(save_frame, text=save["date"].strftime('%A %' + symbol_space + 'I:%M%p, %d %b %Y'),
                                      font=("Tw Cen MT", 9 if windows else 12))
                similar_labels.append(date_label)
                date_label.place(x=390, y=5, anchor='ne')

                # The difficulty mode label
                mode_label = tk.Label(save_frame, text="Difficulty", font=("Tw Cen MT", 12 if windows else 16))
                similar_labels.append(mode_label)
                mode_label.place(x=30, y=45, anchor='w')

                # The difficulty mode
                mode = tk.Label(save_frame, text=save["difficulty"].upper(), font=("Tw Cen MT", 15 if windows else 20, "bold"))
                ...
                avg_game_label.place(x=30, y=62, anchor='w')

                # Creates a delete button on each frame, to delete the save
                delete_button = CustomButton(save_frame, text="Delete", width=60, height=30, font=("Tw Cen Mt", 9 if windows else 12),
                ...
                open_button = CustomButton(save_frame, text="Open", width=60, height=30, font=("Tw Cen MT", 9 if windows else 12),
...
class SetupWindow(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        # 'SETUP GUIDE' label
        help_label = tk.Label(help_container, text="SETUP GUIDE", font=("Tw Cen MT", 15 if windows else 20, "bold"),
        ...
        difficulty_button = tk.Label(difficulty_container, text='NORMAL', font=("Tw Cen MT", 18 if windows else 24, "bold"),
        ...
        reset_board_button = CustomButton(info_frame, text="RESET BOARD", height=40, width=200, align='center',
                                          ...
                                          font=("Tw Cen MT", 15 if windows else 20, "bold"), bg_canvas=theme.GRAY_LIGHT)
        ...
        start_game_button = CustomButton(info_frame, text="GO!", height=150, width=200,
                                         ...
                                         font=("Tw Cen MT", 30 if windows else 40, "bold"), bg_canvas=theme.GRAY_LIGHT)
...
class GameWindow(tk.Frame):
    ...
    def __init__(self, parent, game):
        ...
        last_location = tk.Label(bottom_filler, text="––",
                                 ...
                                 font=("Tw Cen MT", 45 if windows else 60, "bold"), height=2, width=4)
        ...
        last_label = tk.Label(bottom_filler, text="Last Location", fg=theme.WHITE, bg=theme.GRAY_DARK,
                              font=("Tw Cen MT", 18 if windows else 24, "bold"))
        ...
        tk.Label(small_grid_info, text="YOUR\nTERRITORY", justify='right',
                             font=("Tw Cen MT", 10 if windows else 13, "bold"), fg=theme.WHITE,
        ...
        fonts = {">b": ("Tw Cen MT", 11 if windows else 14, "bold"),
                 "default": ("Tw Cen MT", 11 if windows else 14)}
        ...
        turn_status = tk.Label(info_frame, text="PLAYER's", fg=theme.RED,
                               font=("Tw Cen MT", 22 if windows else 30, "bold"), width=12)
        ...
        tk.Label(info_frame, text="TURN", fg=theme.GRAY_LIGHT,
                 font=("Tw Cen MT", 11 if windows else 14, "bold"), bg=theme.GRAY_BLACK).pack()
